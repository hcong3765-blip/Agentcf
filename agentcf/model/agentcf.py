...
import numpy as np
from agentverse.llms.zhipu_embeddings_utils import (
    get_embedding,
...
class AgentCF(SequentialRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        embedding_context = \
            {'agent_type': 'embeddingagent',
             ...
             'llm_chat': {'model': 'glm-4', 'llm_type': 'glm-4',
        ...
        rec_context = \
            ...
            'llm_chat':{'model':'glm-4','llm_type':'glm-4','temperature':self.config['llm_temperature_test'],'max_tokens':self.config['max_tokens_chat'],'api_key_list':self.config['api_key_list'],'current_key_idx': self.config['current_key_idx'],},
    ...
    def load_user_context(self):
        ...
        user_context[0] = {'agent_type':'useragent', 'role_description':{'age': '[PAD]', 'user_gender': '[PAD]','user_occupation':'[PAD]'},'memory_1':['[PAD]'],'update_memory':['[PAD]'],
                     ...
                     'llm':{'model':self.config['llm_model'],'llm_type':self.config['llm_model'],'temperature':self.config['llm_temperature'],'max_tokens':self.config['max_tokens'],'api_key_list':self.config['api_key_list'],'current_key_idx': self.config['current_key_idx']},
                     'llm_chat':{'model':'glm-4','llm_type':'glm-4','temperature':self.config['llm_temperature'],'max_tokens':self.config['max_tokens_chat'],'api_key_list':self.config['api_key_list'],'current_key_idx': self.config['current_key_idx']},
    ...
    def backward(self, system_reasons, batch_user, batch_pos_item, batch_neg_item):
        ...
        for i in range(batch_size):
            if i >= len(item_update_memories):
                print("*" * 10 + f"item update index out of range: {i} >= {len(item_update_memories)}" + "*" * 10 + '\n')
                # 使用默认值或跳过此次更新
                continue
            elif len(item_update_memories[i]) != 2:
                print("*" * 10 + f"item update 解析结果不完整，期望长度2，实际长度{len(item_update_memories[i])}: {item_update_memories[i]}" + "*" * 10 + '\n')
                # 尝试使用单个值或默认处理
                if len(item_update_memories[i]) >= 1:
                    # 使用第一个元素作为正向物品描述，第二个元素作为负向物品描述
                    pos_desc = item_update_memories[i][0] if len(item_update_memories[i]) > 0 else "Default positive item description"
                    neg_desc = item_update_memories[i][1] if len(item_update_memories[i]) > 1 else "Default negative item description"
                    self.item_agents[int(batch_pos_item[i])].update_memory.append(pos_desc)
                    if self.config['update_neg_item']: 
                        self.item_agents[int(batch_neg_item[i])].update_memory.append(neg_desc)
                else:
                    # 完全解析失败，使用默认描述
                    default_desc = "Default item description due to parsing failure"
                    self.item_agents[int(batch_pos_item[i])].update_memory.append(default_desc)
                    if self.config['update_neg_item']: 
                        self.item_agents[int(batch_neg_item[i])].update_memory.append(default_desc)
            else:
    ...
    def logging_during_updation(self, batch_user, system_explanations, user_backward_prompts, pos_item_descriptions_forward, neg_item_descriptions_forward, user_update_descriptions, item_update_memories):
        ...
        for i in range(batch_size):
            ...
            with open(osp.join(path, f'user.{str(user_id)}'), 'a') as f:
                ...
                # 安全地访问item_update_memories，处理索引越界和解析错误
                if i < len(item_update_memories) and len(item_update_memories[i]) >= 2:
                    if self.config['update_neg_item']:
                        f.write(
                ...
                elif i < len(item_update_memories) and len(item_update_memories[i]) >= 1:
                    # 只有一个更新信息可用
                    f.write(
                        f'The positive CD has the following updated information: {item_update_memories[i][0]}\n\n')
                else:
                    # 没有可用的更新信息
                    f.write(
                        f'CD update information not available due to parsing failure.\n\n')
